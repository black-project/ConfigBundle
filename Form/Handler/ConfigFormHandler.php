...
class ConfigFormHandler implements HandlerInterface
{
    ...
    protected function onSave(ConfigInterface $config)
    {
        ...
        if ($config->getName() == 'General')
        {
            if(array_key_exists('organization_name',$config->getValue()))
            {
                $this->constructFooter($config);
            }
        }
    ...
    /**
     * This method constructs and persist a new parameter string generated by organisation_*
     * config's parameters to include in footer site's
     *
     * @return true
     */
    public function constructFooter($config)
    {
        ...
        (
            ...
            'organization_prt_phone',
            'organization_fix_phone',
        );
        ...
        $preData                  = $config->getValue();
        $preData['footer_string'] = $footerString;


        $config->setValue($preData);
